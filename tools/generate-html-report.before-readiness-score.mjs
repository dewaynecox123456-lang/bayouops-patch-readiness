...
const mappings = JSON.parse(
  fs.readFileSync("mappings/default-mappings.json","utf8")
);
...
function parseCsv(text) {
  ...
  const resolvedHeaders = rawHeaders.map(header => {
  ...
  return lines.map(line => {
    ...
    return Object.fromEntries(
      resolvedHeaders.map((h,i)=>[h, values[i] ?? ""])
    );
  });
}
...
<head>
<meta charset="utf-8">
<title>BayouOps Mapping Engine</title>
...
<div class="hero">
<div>
<h1>BayouOps Mapping Engine Dashboard</h1>
<div class="sub">
Flexible inventory normalization • Visibility, not control
</div>
</div>
...
<h2>Normalized Inventory Detail</h2>
...
<div class="footer">
© 2026 BayouOps • BayouFinds.com • Flexible operational readiness visibility
</div>
...
const outfile = path.join(outDir, `bayouops_mapping_dashboard_${stamp}.html`);
